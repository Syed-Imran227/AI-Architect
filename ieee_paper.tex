...
our prompt engineering strategy. The challenge of constraining LLM outputs
to machine-parseable schemas has been addressed through constrained decoding;
Willard and Louf~\cite{efficient_guided_gen} demonstrated that finite-state-machine
token masking enables reliable structured JSON generation from general-purpose
language models without fine-tuning or post-hoc extraction heuristics.

Vastu Shastra as a formal computational constraint has received limited
treatment in academic literature. Rao~\cite{vastu_science} provided a
systematic analysis of directional and spatial rules embedded within the
Vastu Shastra framework, establishing the compass-zone taxonomy that our
engine adopts and extends with a weighted scoring model and an automated
...
The AI Architect platform is organized as a specialized dual-pipeline
architecture, as illustrated in Fig.~\ref{fig:architecture}. Rather than
relying on a single monolithic generation model, the system separates
concerns into a probabilistic reasoning layer and a deterministic geometric
drafting layer.

% ---- Fig 1: Architecture Diagram ----
% PLACEMENT FIX: Moved here to Section III where \ref{fig:architecture} first appears.
\begin{figure}[t]
  \centering
...
% PLACEMENT FIX: Moved here to start of Section IV where \ref{fig:flowchart} first appears.
\begin{figure}[t]
  \centering
...
extraction step~\cite{efficient_guided_gen}.
...
Vastu Shastra guidelines~\cite{vastu_science}. The plot is divided into a
$3{\times}3$ compass-zone grid computed from the user-specified entrance
direction. Each room centroid is computed as:
\begin{equation}
...
The user interface of the interactive blueprint editor is shown in
Fig.~\ref{fig:ui_editor}.

% ---- Fig 3: UI Editor Screenshot ----
% PLACEMENT FIX: Moved here to Section IV.F where \ref{fig:ui_editor} first appears.
\begin{figure}[t]
  \centering
...
&
Couples graph semantic models with wall-oriented generation to enforce boundary constraints.
&
...
% ---- Fig 4: Vastu Analysis ----
% PLACEMENT FIX: Moved here to Section VI.B where \ref{fig:vastu_analysis} first appears.
\begin{figure}[t]
  \centering
...
%  REFERENCES
%
%  CORRECTIONS MADE (per reviewer decision):
%  [3] wallplan  -- FIXED: Prior version cited "Zhang et al., ICCAD 2022"
%                   with DOI 10.1145/3508352.3549412. The actual WallPlan paper
%                   is by Sun et al., published in ACM Transactions on Graphics
%                   (SIGGRAPH 2022), DOI: 10.1145/3528223.3530135.
%  [9] efficient_guided_gen -- REPLACED: Prior citation "StructuredLLM,
%                   arXiv:2309.10379" was fabricated; that arXiv ID belongs to
%                   a speech-enhancement paper (PDPCRN). Replaced with the real
%                   and widely-cited Willard & Louf (2023), arXiv:2307.09702,
%                   on constrained JSON generation via finite-state machines.
%  [10] vastu_science -- REPLACED: "Bhatt & Sharma, IJACSA 2023" is
%                   unverifiable and absent from all academic databases.
%                   Replaced with Rao (2000), a traceable scholarly monograph.
% ====================================================================
...
% [3] CORRECTED -- see header note above.
\bibitem{wallplan}
J.~Sun, W.~Wu, L.~Liu, W.~Min, G.~Zhang, and L.~Zheng,
``WallPlan: Synthesizing Floorplans by Learning to Generate Wall Graphs,''
\textit{ACM Trans. Graphics (Proc. SIGGRAPH)}, vol.~41, no.~4, Article~1,
Jul.~2022, doi: 10.1145/3528223.3530135.
...
% [9] REPLACED -- see header note above.
\bibitem{efficient_guided_gen}
B.~T.~Willard and R.~Louf,
``Efficient Guided Generation for Large Language Models,''
\textit{arXiv preprint arXiv:2307.09702}, 2023,
doi: 10.48550/arXiv.2307.09702.

% [10] UPDATED: Replaced Rao (2000) monograph with a more recent, peer-reviewed
%      journal article (Dash & Joshi, 2021) that is DOI-linked and verifiable
%      on SciEPub and ResearchGate.
\bibitem{vastu_science}
S.~Dash and M.~Joshi,
``A Comparative and Critical Analysis of Application of Vastu Shastra's Concepts
with Philosophy, Psychology, Feng Shui, Seismic Design and Contemporary Architecture
Design Principles: A Review,''
\textit{Applied Ecology and Environmental Sciences}, vol.~9, no.~9, pp.~838--845,
2021, doi: 10.12691/aees-9-9-8.

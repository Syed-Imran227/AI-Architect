...
reasoning with programmatic coordinate constraints.
...
injected into a chain-of-thought (CoT) prompt template at runtime. The CoT
strategy forces the LLM to explicitly calculate available square footage before
assigning spatial coordinates. The prompt strictly enforces:
\begin{enumerate}[leftmargin=*, label=\arabic*., itemsep=0pt, topsep=2pt]
  ...
  \item Output as strict JSON with fields: name, x,
        y, width, height, and a nested
        furniture array with room-relative coordinates.
\end{enumerate}
...
\end{equation}
Rooms generated below a minimum threshold of 150\,sq.\,ft are algorithmically
expanded to prevent degenerate or unbuildable geometry. Additionally, all
required JSON fields and data types are verified through a schema
validation layer before returning the final API response.
...
WALLS (filled polylines representing structural boundaries),
LABELS (text markers anchored at room centroids),
DOORS (swing arc entities calculated from wall intersections), and
DIMENSIONS (linear dimension constraints). The export scale strictly maps
internal pixel coordinate units to real-world feet at a 1:1 ratio, ensuring
that the generated drawing can be immediately utilized for structural
engineering and municipal permitting workflows without manual rescaling.
...
AI Architect (Ours)
&
Llama-3-70B CoT prompting $\rightarrow$ JSON coords; 10-rule Vastu engine; Auto-Fix loop; Pillow; ezdxf DXF; React~19 SVG editor.
&
91\% Vastu convergence; 88\%+ coverage; DXF validated in AutoCAD~2023; 60\,FPS drag editing. \\
...
or overlapping room geometry. The rule-based Vastu compliance engine operates in
$O(n)$ time complexity relative to the number of rooms, correctly identifying
non-compliant spatial placements with 100\% deterministic consistency. A sample of the compliance reporting and visualization is shown in Fig.~\ref{fig:vastu_analysis}.
...
coordinates when provided with explicit corrective feedback.
...
single-iteration convergence rate to full rule compliance.
...
vector coordination, this architecture sets a new standard for functional
generative design. Future work will aim to extend the platform toward real-time
3D WebGL visualization and the integration of localized municipal bylaw
compliance checking.
